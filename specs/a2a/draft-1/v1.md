...
`/specs/a2a/draft-1/generate_vectors.py`, which recursively serializes `int` values as
...
|--------|------------|-----------|
| Read envelope bodies | **CAN** | v1 envelopes are signed but NOT encrypted. Marketplace prices, task descriptions, and negotiation terms are visible to the relay operator. Implementations with confidentiality requirements MUST use TLS 1.3 at minimum; end-to-end encryption is deferred to v1.5. |
...
v1 envelopes are **signed but not encrypted**. This is an explicit, documented tradeoff:
v1 is an open-specification demo and integration target, not a production-confidentiality
system. The full threat model for production-confidentiality use cases is addressed in
the planned v1.5 encryption extension, which will add anonymous-encryption (`anon_crypt`)
and authenticated-encryption (`auth_crypt`) modes using `crypto_box` (NaCl/libsodium
primitives, approximately 200 lines of implementation code). Until v1.5 is published,
implementations with confidentiality requirements SHOULD NOT transmit sensitive data in
A2A envelope bodies unless TLS 1.3 is used for the transport layer AND the relay operator
is trusted.
...
- `/specs/a2a/draft-N/` — a working draft, subject to change until promotion.
- `/specs/a2a/vN/` — a published version, frozen at the moment of promotion.

**A URL of the form `/specs/a2a/vN/` is frozen at publish. It MUST NOT be modified after
promotion.** CI tooling in the `air-site` repository MUST block any pull request that
modifies files under `specs/a2a/vN/` (for any integer N). This permanence guarantee allows
...
| `vN+1` (major) | Breaking changes. Assigned a new URL (`/specs/a2a/v2/`). Implementations targeting v1 are NOT required to support v2. The v1 URL remains permanently accessible. |
...
2. JSON Schema validation: all example envelopes in `specs/a2a/draft-1/examples/` validate
   against `specs/a2a/draft-1/envelope.schema.json`.
...
at `agentidentityregistry.org/specs/a2a/service-types` as a lightweight alternative to
...
`/specs/a2a/v1/test-vectors.json`.**
...
```
https://agentidentityregistry.org/specs/a2a/v1/test-vectors.json
```

During the `draft-1` phase, the vectors are at:

```
https://agentidentityregistry.org/specs/a2a/draft-1/test-vectors.json
```

The vectors file is generated by `/specs/a2a/draft-1/generate_vectors.py` and covers the
